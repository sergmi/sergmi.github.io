function saveCurrentScore(){/*
	...
	localStorage.setItem('battleGameHighscores', JSON.stringify(null));
}
...
let enemy_definitions = ["sunny","cloudy","rainy","snowy","flamy"];
let enemy_instances = ["wizard","magician","theurgist","enchanter","conjuror"];
...
exit_button.addEventListener("click", finishGame, false);
...
function startCounter(){
	...
	weapon_choise.className = "weapon-choise-button";
	exit_button.className = "exit-button";
}

function createGameField(){
	player_influence_image.removeEventListener("animationend", createGameField, false);
...
function applyPlayerWeapon(){
	if(weapon_name === "struggle-weapon-1"){
		...
		if(enemy_health<1){
			...
			player_count += 1;
			player_influence_image.addEventListener("animationend", createGameField, false);
		}
		enemy_bar.go( 100-enemy_health );
	};
...
function applyEnemyWeapon(){
	...
	if(player_health<1){
		...
		enemy_influence_image.addEventListener("animationend", hidePlayerImage, false);
	}
	player_bar.go( player_health );
}
...
function showResult(){
	...
	if(player_reply.answer.value === String(eval(figures[0]+task_operator+figures[1]))){
...
function hideTask(){
	...
	if(player_reply.answer.value === String(eval(figures[0]+task_operator+figures[1]))){applyPlayerWeapon();}
...
function hidePlayerImage(){
	enemy_influence_image.removeEventListener("animationend", hidePlayerImage, false);
...
function hideGame(){
	...
	weapon_choise.removeEventListener("click", loadModalDialog, false);
	weapon_choise.className = "weapon-choise-button-hidden";
	exit_button.className = "deleted";
...
function finishGame(){
		if(player_count > 0){	//  condition
			saveCurrentScore();	
			if(task_field.className == "task"){hideTask();}
			endGame();
		}
		else{
			if(task_field.className == "task"){hideTask();}
